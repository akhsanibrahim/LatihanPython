...
def get_dht11_data():
	data1 = sensor1.read()
	hasil1 = "Temperature	: "+str(data1.temperature())+" C"+'\r'+"Humidity	:"+str(data1.humidity())+" %"
	return hasil1
#def get_cpu_temp():
...
def send_dht11_continue(send_to_who,continue_or_not = True):
    ...
    if continue_or_not:
        ...
        node.send(str(get_dht11_data()))
        time.sleep(0.2)
        node.set(node.freq,node.addr_temp,node.power,node.rssi)
        timer_task = Timer(seconds,send_dht11_continue,(send_to_who,))
        timer_task.start()
    else:
        node.send_to = send_to_who
        node.addr_temp = node.addr
...
try:
    ...
    print("Press \033[1;32ms\033[0m   to send temperature & humidity every 10 seconds")
